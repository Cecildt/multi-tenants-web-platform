...
- [ ] 2.6 Update the README "Development" section: replace the Turso CLI steps with `wrangler d1 create`, `db:generate` (in `tenants-db-lib`) and the migrate/seed scripts (in `astro-web-platform`). Verify: no `turso` commands remain in `README.md`.
...
- [ ] 4.1 Rewrite `design-docs/deployment.md`: install `wrangler` (not `@cloudflare/wrangler`), create D1, run `db:migrate:remote` and `db:seed:remote`, then build and deploy, in that order. Verify: the doc has no `@cloudflare/wrangler` or Turso references.
